...
$('select.item_name_dropdownone').on('change', function() {
    item_change_func_new_invoice();
});
...
function item_change_func_new_invoice() {
    ...
    $('#new_invoice_form_f').find('.product_id_invoice_row').each(function(product_id_index) {
        var product_id = parseInt($(this).find('.item_line_id').val());
    ...
    if (product_exists) {
        ...
        /* var invoice_form = $('#new_invoice_form_f');
                 console.log(invoice_form, jei_product_index_er_quantity_update_hobe);
        ...
        $.get('include/item_description.php?class_id=one&item_id=' + item_name_dropdownone_val + '', function(data) {
            ...
            $('#new_invoice_form_f').find('.product_id_invoice_row').each(function(product_id_index) {
                invoice_calculate_func(product_id_index, $('#new_invoice_form_f'));
            });
            invoice_form_each_func();
        });
    }
...
function invoice_form_each_func() {
    $('.lineRemove').each(function(i) {
        $(this).click(function() {
            ...
            var arr = [];
            $('#new_invoice_form_f').find('.item_line_id').each(function(index) {
                arr[index] = $(this).val();
            });
            $('#item_ids').val(arr);

            if (!$('#new_invoice_form_f').find('.product_id_invoice_row').length) {
                $('.grand_line_total').text('0');
                $('.invoice_bill').text('0');
                $('.invoice_subtotal').text($('.previus_due_total').text());
                $('.invoice_due_total').text($('.previus_due_total').text());
                $('.invoice_due_total_input').val($('.previus_due_total').text());
            }
            if (i == 0) {
    ...
    $('#new_invoice_form_f').each(function() {
        ...
        // });
        var arr = [];
        invoice_form.find('.item_line_id').each(function(index) {
            arr[index] = $(this).val();
        });
        $('#item_ids').val(arr);
...
function invoice_calculate_func(price_index, invoice_form) {
    ...
    var item_line_one_price;
    $('.item_line_one_price').each(function(index) {
        if (index == price_index) {
            item_line_one_price = this.innerHTML * 1;
        }
    });
